...
const fetchLatestBaileysVersion = async (options = {}) => {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(`Failed to fetch latest Baileys version: ${response.statusText}`);
		}
		...
		const versionMatch = versionLine && versionLine.match(/const version = \[(\d+),\s*(\d+),\s*(\d+)\]/);
		...
		return {
			version: typeof baileysVersion !== 'undefined' ? baileysVersion : [2, 3000, 0],
			isLatest: false,
			error
		};
	}
